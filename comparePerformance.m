...
for i = 1:nargin
    agent(i) = processData(varargin{i}); %#ok<AGROW>
end
...
for i = 1:size(files,1)
    s = load(files(i).name,'trainingStats');
    stats = s.trainingStats;
    agent.AverageReward(:,i) = stats.AverageReward;
    agent.EpisodeQ0(:,i) = stats.EpisodeQ0;
    agent.EpisodeIndex = stats.EpisodeIndex;
    agent.ElapsedTime = stats.Information.ElapsedTime;
end
